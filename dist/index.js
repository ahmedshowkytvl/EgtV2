...
var init_db = __esm({
  "server/db.ts"() {
    "use strict";
    init_schema();
    DATABASE_URL = process.env.DATABASE_URL || "postgresql://egsite:Pass2020@74.179.85.9:5432/egsite_db?sslmode=disable";
    if (!DATABASE_URL) {
      throw new Error(
    ...
    pool = new Pool({
      ...
      ssl: false
      // Disable SSL for self-signed certificate issues
    });
  ...
  "server/storage.ts"() {
    ...
    DatabaseStorage = class {
      ...
      async listHotels(active) {
        try {
          console.log("\u{1F3E8} Listing hotels with active filter:", active);
          if (active !== void 0) {
            const result2 = await db.select().from(hotels).where(eq(hotels.status, active ? "active" : "inactive")).orderBy(desc(hotels.id));
            console.log("\u2705 Found hotels with active filter:", result2.length);
            return result2;
          }
...
(async () => {
  try {
    ...
    if (dbInitialized) {
      ...
      console.log(
        "\u{1F4E6} Using fallback storage due to database connection issues."
      );
    }
    try {
      await setupAdmin();
    ...
    try {
      ...
      app.use(
        "/uploads",
        express3.static(path8.join(process.cwd(), "public/uploads"))
      );
    ...
    app.get("/admin-test", (req, res) => {
      res.sendFile(
        path8.join(process.cwd(), "client", "public", "admin-test.html")
      );
    });
    (async () => {
      try {
